...
const order = ref([]);
let urlOrder = "http://localhost:8080/vuejs-server/api.php/show_order";
let urlChangeStatus =
  "http://localhost:8080/vuejs-server/api.php/change-status";
        ...
        <tbody>
          <tr v-for="(item, index) in order" :key="index">
            ...
            <td>
              ...
              <span v-else-if="item.status == 4" class="badge text-bg-success">
                Đang giao hàng
              </span>
            ...
            <td>
              <RouterLink
                :to="`/don-hang-chi-tiet/${item.order_id}`"
                class="btn btn-info"
              ...
              <button
                ...
                @click="handleCancel(item.order_id)"
              >
                Hủy đơn
              </button>
            </td>
