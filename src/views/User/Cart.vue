...
let urlCartUpdate = "http://localhost:8080/vuejs-server/api.php/cart-update";
      ...
      <table class="table">
        ...
        <tbody>
          ...
          <tr v-else v-for="(item, index) in cartDetail" :key="index">
            ...
            <td>
              <span>{{ convertPrice(Number(item.price)) }} vnđ</span>
            </td>
            <td>
              <div class="d-flex">
                <button
                  class="btn btn-sm btn-info"
                  @click="handleUpdateCart('decrease', item.product_id)"
                >
                  -
                </button>
                ...
                <button
                  class="btn btn-sm btn-info"
                  @click="handleUpdateCart('increase', item.product_id)"
                >
                  +
                </button>
              </div>
            ...
            <td>
              {{ convertPrice(Number(item.price) * Number(item.quantity)) }} vnđ
            </td>
            <td>
              <button
                @click="handleUpdateCart('delete', item.product_id)"
          ...
          <tr v-if="cartDetail.length != 0">
            ...
            <td>
              <RouterLink to="/thanh-toan" class="btn btn-warning">
                Thanh toán
              </RouterLink>
            </td>
          </tr>
