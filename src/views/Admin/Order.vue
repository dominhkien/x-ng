...
let urlOrderAll = "http://localhost:8080/vuejs-server/api.php/show_order_admin";
let urlChangeStatus =
      ...
      <tbody>
        <tr v-for="(item, index) in orderList" :key="index">
          ...
          <td>
            <select
              class="form-control"
              v-model="statusOrderMap[item.order_id]"
              @change="
                handleStatusChange(item.order_id, statusOrderMap[item.order_id])
              "
            >
